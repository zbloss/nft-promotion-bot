...
class OpenseaAPI:
    ...
    def __init__(self, asset_owner: str = None, base_api: str = 'https://api.opensea.io/api/v1'):
    ...
    def get_collections(self, maximum_returned_collections: int = 300) -> list:
        ...
        assert response.status_code == 200, f'Unable to retrieve collections: {response.status_code} | {response.text}'
        return (response.status_code, response.json())

    def _get_assets(self, collection: str = None, offset: int = 0, limit: int = 50) -> list:
        '''
        ...
        response = requests.get(url, params=querystring)
        assert response.status_code == 200, f'Unable to retrieve assets: {response.status_code} | {response.text}'
        return (response.status_code, response.json())
